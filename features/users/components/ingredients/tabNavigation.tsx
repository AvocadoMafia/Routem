    ...
    <div className="flex items-center gap-8 border-b border-grass mb-8">
      ...
      {mode === 'self' && (
        <button 
          ...
          className={`flex items-center gap-2 pb-4 text-sm font-bold transition-all relative cursor-pointer ${activeTab === 'history' ? 'text-accent-1' : 'text-foreground-1 hover:text-foreground-0'}`}
        >
          <MdHistory size={18} />
          <span>HISTORY</span>
          {activeTab === 'history' && <motion.div layoutId="activeTab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-accent-1" />}
        </button>
      )}
    </div>
  )
