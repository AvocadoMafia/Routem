...
| H-14 | `scripts/` 配下の console.* 文言が混在 (EN/JA)。EN: generateRecommendations / seedExchangeRates。JA: setupMeilisearch / updateBudgetInUsd / updateExchangeRates (部分)。ops 担当者が JP 話者で統一すると障害解析が楽になる。**優先度低** (CLI のみで本番ユーザーに見えない) |
| H-15 | race で紛れ込んだ commit `5d0c9d7` (メッセージは "useShareフック..." だが実体は Engineer_F の enum 改修) を main merge 前に `git rebase -i` で squash or reword 検討。内容自体は正しいので merge はできる |
| H-16 | `features/` の service/repository レイヤに直接ユニットテストを追加 (現状ほぼ無い)。 prisma mock と supabase mock の scaffolding から必要なので、別 PR で設計から着手推奨。 Bug#3 で handleError.ts の分岐が増えたため、 service 側の throw パターンが想定通り status に変換されるかも併せて検証したい |
| H-17 | API handler で validateUser helper (未実装) を導入して各 route 先頭の `const { data: { user }, error } = await supabase.auth.getUser(); if (error \|\| !user) throw new Error("Unauthorized");` を共通化。現状 15+ 箇所でコピペされている |
| H-18 | Dockerfile の `npm install` 運用は darwin-arm64 ホスト lockfile ↔ linux container の platform 差分対策（`npm ci` では `Missing: @swc/helpers@0.5.21 from lock file` 等で abort した実績あり）。reproducibility が若干落ちるため、将来的には CI/Docker 側で Linux 向け `npm install` → lockfile を export し、ホスト lockfile を上書きする workflow を組んで `npm ci` に回帰するのが理想。commit `fb46783` + `.claude/proxy-ops-review.md` 近傍（将来作成する operational-notes）に経緯を記録する |
| H-19 | 回帰防止として image 内 `node_modules/esbuild/package.json` version を CI で assert するテストを追加（今回 Feb 25 の古い base image cache が image に焼き込まれていた事象の再発を早期検知）|
| H-20 | runner image を `node:22-slim` から `node:22-alpine` にするか、runner stage を production dependency のみで再構築する multi-stage build に切替えて image サイズ削減。現状 `routem-app:latest` は 2.85GB と大きい |
